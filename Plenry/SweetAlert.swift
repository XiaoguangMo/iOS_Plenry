...
public class SweetAlert: UIViewController {
    ...
    let kHeightMargin: CGFloat = 20.0
    let KTopMargin: CGFloat = 80.0
    ...
    private func resizeAndRelayout() {
        ...
        if buttons.count == 2 {
            totalWidth = buttonRect[0].size.width + buttonRect[1].size.width + kWidthMargin/2 + 40.0
        }
        else{
        ...
        for i in 0 ..< buttons.count {
                ...
                buttonX = buttons[i].frame.origin.x + kWidthMargin/2 + buttonRect[i].size.width + 20.0
